...
- Сборка входных данных из БД (H2H-запрос к `Match`, даты/дельты) — на **этап 6**, где потребляется
...
- Пул: уровень + не в паре на турнире + гео r77 + `(A+B)/2 ≤ limit` + **пол**
  (MD → партнёр `M`, WD → `F`, XD → противоположный пол; `PairCompositionService`, `player.sex` — этап 5).
- В строке кандидата — тип будущей пары с пользователем («мужская пара» / «микст» и т.д.).
- Два блока + score; boost ×1.2 для «Уже играли успешно».
- Пользователь: поиск своего профиля **или** привязанный «Мой профиль» (если этап 8 уже готов — иначе только поиск, см. DoD этапа 8).
...
**Зависимости:** этапы 2 (регистрация турниров), 3 (`PartnerScoreService`), 5 (пол).
...
- Интеграция: подбор партнёра (этап 7) и лайв (этап 9) используют привязку по умолчанию.
...
**Зависимости:** этап 4 (bot shell, поиск). **Желательно после:** этап 7 (партнёры) или параллельно.
