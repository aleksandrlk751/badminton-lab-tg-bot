...
> **Статус:** этапы 0–5 завершены (worker: слепок r77 + пол игрока, core: метрики + `PairCompositionService`, bot: поиск/карточка/соперники).  
> Следующий шаг — этап 6 (H2H для пар), затем партнёры (7) → привязка TG (8) → лайв (9) → VPS (10). График рейтинга — v2.  
> Бриф, план, spike-отчёт — в [`docs/`](.). Навигация по зонам проекта для доработок — [`ROUTING.md`](ROUTING.md).
...
| Telegram-бот: поиск, карточка (форма, игровой акцент), соперники (H2H и график — заглушки) | ✅ этап 4 |
| Пол игрока (`player.sex`), справочник + fallback, `PairCompositionService` | ✅ этап 5 |
...
| `SNAPSHOT_SYNC_SEX_ON_STARTUP` | `false` | Разовая синхронизация пола без слепка; worker завершит процесс после sync |
| `SNAPSHOT_INFER_SEX_FROM_NAMES_ON_STARTUP` | `false` | Офлайн fallback пола по ФИО (без справочника и профилей); worker завершит процесс |
...
Шаг в конце слепка (`PlayerSexSyncService`): справочник singles + doubles (M/F), fallback из локальных дисциплин, участия из SSR-профиля (**все регионы**), затем **ФИО из БД** (офлайн). Подробности — [`spike-parser.md`](spike-parser.md) §7, [`PLAN.md`](PLAN.md) этап 5.

Разовый прогон без слепка (полный sync, нужен интернет):
...
Только офлайн fallback по ФИО (без интернета):

```powershell
$env:SNAPSHOT_INFER_SEX_FROM_NAMES_ON_STARTUP='true'
$env:SNAPSHOT_RUN_ON_STARTUP='false'
.\mvnw.cmd -pl worker spring-boot:run
```

Покрытие на полном прогоне r77 (2026-07-23): **~99,96%** игроков с заполненным `sex` (4913/4915); без пола — неоднозначное имя или отсутствие ФИО.
...
docs/ROUTING.md  — навигация по зонам проекта (что читать для конкретной задачи)
...
AGENTS.md        — инструкции для AI-агентов (точка входа → ROUTING.md)
```
...
| Документ | Назначение |
|---|---|
| [`BRIEF.md`](BRIEF.md) | Продуктовые решения, модель данных, ограничения источника |
| [`PLAN.md`](PLAN.md) | Этапы реализации, DoD, зависимости |
| [`FORMULAR.md`](FORMULAR.md) | Формулы и константы (дефолты в коде) |
| [`ROUTING.md`](ROUTING.md) | **Какой код и какие docs читать** под конкретную задачу (парсер, bot, метрики, миграция…) |
| [`spike-parser.md`](spike-parser.md) | Контракты парсера, fixtures, выводы spike |
| [`messages/`](messages/) | Согласованный UX текстов и клавиатур бота |
